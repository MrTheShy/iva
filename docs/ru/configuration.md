...
| `ASSISTANT_TIMEZONE`       | `Asia/Almaty`                  | Имя из базы IANA. Задаёт даты дневных транскриптов, два systemd watchdog-таймера, шесть внутрипроцессных eve schedules и дату/время, которые Iva видит на каждом ходе. Экспортируется как `TZ`.                                                                                                      |
