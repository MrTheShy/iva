...
| `ASSISTANT_TIMEZONE`       | `Asia/Almaty`                  | IANA name. Sets daily-transcript dates, two systemd watchdog timers, six in-process eve schedules, and the date/time Iva sees each turn. Exported as `TZ`.                                                                                                                                                     |
...
| `IVA_APP_BEARER`           | _(empty)_                      | Secret the Android and Wear OS voice apps present to `POST /eve/v1/app`. Empty means nobody gets in — set it only when you publish that route ([app.md](./app.md)). Never reuse `ASSISTANT_BEARER`: that one is the server's internal secret and must not ship inside an APK.                                    |
