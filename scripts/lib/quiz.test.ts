...
// Оси и их полюса (буквы) — зеркало quiz.ts (POLES/AXIS_ORDER там приватны). Первый
...
function answersFor(code: string) {
  return QUIZ.map((q) => {
...
void test("scoreQuiz возвращает каждый из 16 кодов ровно один раз на крайних ответах", () => {
...
void test("ARCHETYPES содержит ровно 16 кодов — те же, что перечисляет скоринг", () => {
  assert.deepEqual(new Set(Object.keys(ARCHETYPES)), new Set(ALL_CODES));
  assert.equal(Object.keys(ARCHETYPES).length, 16);
});

void test("у каждого кода портрет и персона непусты на ru/en, персона <= 800 символов", () => {
  for (const code of ALL_CODES) {
...
void test("тай по оси (сумма 0) решается в пользу первого полюса", () => {
