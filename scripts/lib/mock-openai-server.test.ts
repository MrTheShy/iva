...
import { startMockOpenAiServer } from "./mock-openai-server.ts";

type CompletionResponse = {
  choices: Array<{ message: { content: string } }>;
};

async function complete(baseUrl: string, body: unknown): Promise<Response> {
...
void test("non-streamed completion answers REPLICA_OK by default", async () => {
...
void test("streamed completion emits SSE chunks and [DONE]", async () => {
  const mock = await startMockOpenAiServer();
  try {
    const res = await complete(mock.baseUrl, {
      model: "iva-replica",
...
void test("marker is echoed back from the replayed transcript", async () => {
  const mock = await startMockOpenAiServer();
  try {
    const res = await complete(mock.baseUrl, {
      model: "iva-replica",
