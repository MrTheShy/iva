...
// 25 минут, не 10: цель таймера — поймать МОЛЧАЛИВОЕ зависание (#104), а не убить
// живой ход. 2026-08-13 ночной роллап тяжёлого дня активно работал (17 и 28 шагов
// по usage.jsonl, ~110k токенов входа на шаг) и был убит 10-минутным таймером
// дважды — ложное срабатывание. Настоящий hang не делает НИ ОДНОГО шага часами,
// так что 25 минут ловят его так же надёжно; 2×25 (основной ход + retry на свежей
// сессии) плюс хвост остаются под часовым SIGTERM schedule-runner'а.
export const DEFAULT_TURN_TIMEOUT_MS = 25 * 60_000;
