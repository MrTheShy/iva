...
- 🧪 **Общие golden-фикстуры для парных парсеров** - frontmatter и fence-сканер секций реализованы дважды (TypeScript и Python) и однажды уже ломались синхронно; теперь оба прогоняются по одному каталогу фикстур `scripts/autograph/tests/golden/` в обоих тест-раннерах - расхождение диалектов ловится тестом, а не ночным пайплайном.
